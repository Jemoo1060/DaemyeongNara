	...
	$("[id^='updateBtn_']").click(function(){
		...
		if(!result){
			return false;
			  ...
			  location.replace("answerUpdate?answerCn=" + obj +"&answerNum="+answerNum+"&bbiCode=${bbdto.bbiCode}&bbNum=${bbdto.bbNum}");
          ...
          <section id="table1" style="margin-top: 30px; margin-left: 10px;">
              <table border="1">
                ...
                <tr>
                  <td colspan="2" style="text-align: left;">
                   	 작성자:관리자&nbsp;  작성일:${regDate}&nbsp;    조회수:${bbdto.bbRdcnt}
                  </td>
                </tr>
                <tr>
           ...
           <section id="button" style="position: relative;">
             ...
             <c:if test="${mbrGrade == '0101'}">
	              <input type="button" value="수정" onclick="document.location.href='bbUpdatePage?bbTitl=${bbdto.bbTitl}&bbCn=${bbdto.bbCn}&bbNum=${bbdto.bbNum}&bbiCode=${bbdto.bbiCode}&pageNo=${pageNo}&cutPageNo=${cutPageNo}'">
		              ...
		              <c:forEach var ="cnt" begin = "0" end = "${bbaList.size()-1}">
						<tr>
		                  ...
		                  <c:if test="${mbrId == mbrList[cnt].mbrId}">
		                  	...
		                  	<input type="button" value="댓글 삭제" style="color:white; background-color:rgb(79,129,189); padding: 3px;" onclick="document.location.href='answerDelete?answerNum=${bbaList[cnt].answerNum}&bbiCode=${bbdto.bbiCode}&bbNum=${bbdto.bbNum}'">
		                  </c:if>                    	
		                  </td>
		                </tr>
		              </c:forEach>
