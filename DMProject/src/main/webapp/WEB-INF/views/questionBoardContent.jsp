	...
	$("[id^='updateBtn_']").click(function(){
		...
		if(!result){
			return false;
			  ...
			  location.replace("answerUpdate?answerCn=" + obj +"&answerNum="+answerNum+"&bbiCode=${bbdto.bbiCode}&bbNum=${bbdto.bbNum}&writerId=${writerId}");
		              ...
		              <c:forEach var ="cnt" begin = "0" end = "${bbaList.size()-1}">
						<tr>
		                  ...
		                  <c:if test="${mbrId == mbrList[cnt].mbrId}">
		                  	...
		                  	<input type="button" value="댓글 삭제" style="color:white; background-color:rgb(79,129,189); padding: 3px;" onclick="document.location.href='answerDelete?answerNum=${bbaList[cnt].answerNum}&bbiCode=${bbdto.bbiCode}&bbNum=${bbdto.bbNum}&writerId=${writerId}'">
		                  </c:if>                    	
		                  </td>
		                </tr>
		              </c:forEach>
