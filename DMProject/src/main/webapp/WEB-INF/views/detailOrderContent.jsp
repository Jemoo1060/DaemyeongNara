...
$(document).ready(function(){
	...
	$('#detailOrderAddr1').text(addr1);
	$('#detailOrderAddr2').text(addr2);
});
</script>
<title>회원 관리</title>
</head>
<body>
    ...
    <section id="main_section">
        <div id="mainContent">
            <h1 id="orderContentmainfont">상세주문 내역</h1>
            <table class="orderContentTable">
                ...
                <tr>
                    <td width="80px">${orderDto.orderCode}</td>
                    <td width="110px">${orderDto.orderDate}</td>
                    <td width="330px">${orderContent}</td>
                    <td>${orderDto.totalPc}원</td>
                    <td width="120px">${memberDto.mbrId}</td>
                    <c:choose>
                    	<c:when test="${orderDto.progrsSttus == '0501'}">
                    		<td id="orderContentdelivery">결제완료</td>	
                    	</c:when>
						<c:when test="${orderDto.progrsSttus == '0502'}">
                    		<td id="orderContentdelivery">배송준비중</td>	
                    	</c:when>
                    	<c:when test="${orderDto.progrsSttus == '0503'}">
                    		<td id="orderContentdelivery">배송중</td>	
                    	</c:when>
                    	<c:when test="${orderDto.progrsSttus == '0504'}">
                    		<td id="orderContentdelivery">배송완료</td>	
                    	</c:when>
                    	<c:when test="${orderDto.progrsSttus == '0505'}">
                    		<td id="orderContentdelivery">취소대기중</td>	
                    	</c:when>
                    	<c:otherwise>
                    		<td id="orderContentdelivery">취소완료</td>	
                    	</c:otherwise>       	
                    </c:choose>
            ...
            <table class="orderContentTable">
                ...
                <c:forEach var="cnt" begin = "0" end="${prdList.size()-1}">
	                <tr>
	                    <td>${prdList[cnt].prdCode}</td>
	                    <td>${prdList[cnt].prdName}</td>
	                    <td>${prdList[cnt].prdPc * oDList[cnt].detailOrderQy}원</td>
	                    <td>${oDList[cnt].detailOrderQy}</td>
	                </tr>
                </c:forEach>
            </table>
            <table class="orderContentTable">
                <tr>
                    ...
                    <th>연락처</th>
                    <td>${deliveryDto.dlvyCttpc}</td>
                </tr>
                <tr>
                    <th>우편번호</th>
                    <td colspan="3">${deliveryDto.dlvyZip}</td>
                </tr>
                <tr>
                    <th>주소</th>
                    <td colspan="3" id="detailOrderAddr1"></td>
                </tr>
                <tr>
                    <th>상세주소</th>
                    <td colspan="3" id="detailOrderAddr2"></td>
                </tr>
            </table>
            <table class="orderContentTable">
                <tr>
                    <th width="130px">요청사항</th>
                    <td>${orderDto.demand}</td>
                </tr>
            </table>
            <div id="orderContentbuttonBox">
                <input id="orderButton" type="button" value="주문목록 조회" onclick="document.location.href='orderInfo?mbrCode=${mbrCode}'">
