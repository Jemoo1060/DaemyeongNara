...
$(document).ready(function(){
	...
	$("[id^='lockBtn_']").click(function(){
		
		var id = this.id;
		var cntArray = id.split("_");
		var bbNum = cntArray[1];
		var writerId = cntArray[2];
		
		$("#pwdInput_" + bbNum + "_" + writerId).show();
		$("#pwdBtn_" + bbNum + "_"+ writerId).show();   
	});
	
	
	$("[id^='pwdBtn_']").click(function(){
		...
		var obj = $("#pwdInput_" + bbNum + "_" + writerId).val();
		var leng = obj.length;
		...
		if(obj == ""){
			...
			location.replace("bbPwdCheck?bbiCode=${bbiCode}&bbNum="+bbNum+"&bbPwd="+obj+ "&writerId="+writerId+"&pageNo=${pageNo}&cutPageNo=${cutPageNo}");
		}
...
$(document).ready(function(){
    ...
    $('#btn1').click(function(){
        $('.menuSelect').html("<div id=\"categoryBtn\"><a href = \"prdList?pclCode=0101\">문구류</a><a href = \"#\">완구류</a><a href = \"#\">제지류</a><a href = \"#\">잡화/펜시류</a><a href = \"#\">미술/음악/체육</a><a href = \"#\">기타</a></div>");
    });
    ...
    <div class="content-box">
      ...
      <div id="main">
        ...
        <div style="margin-top: 100px; margin-left: 10px;">
          <a href="#"><img src="${path}/resources/images/question.png"></a>
        </div>
        ...
        <div class="table_box" style="margin-top: 45px;">
            	...
            	<c:when test="${bbList.size() > 0}">
	            	<c:forEach var = "cnt" begin = "0" end = "${bbList.size()-1}">
		            	<tr>
			                ...
			                <c:choose>
			                	<c:when test="${bbList[cnt].bbOthbcAt == 'Y'.charAt(0)}">
			                	...
			                	<c:otherwise>
			                		<td class="notice_title">
			                  			<a id ="lockBtn_${bbList[cnt].bbNum}_${mbrList[cnt].mbrId}">
			                  				<img style="width : 15px; height : 15px;" src="${path}/resources/images/lock.png">&nbsp;${bbList[cnt].bbTitl}
			                  			</a>
			                  			<input id="pwdInput_${bbList[cnt].bbNum}_${mbrList[cnt].mbrId}" type = "password" style = "width : 90px; height:20px;" placeholder="숫자 4자리" maxlength="4">
			                  			<input ID= "pwdBtn_${bbList[cnt].bbNum}_${mbrList[cnt].mbrId}" type = "button" value= "입력" style = "height:25px;">
			                		</td>
			                	</c:otherwise>
			                </c:choose>
          ...
          <section style="position: relative;">
            <div style="margin-top: 30px;">
	            <c:if test="${mbrCode != null}">
	            	<input id="write_button" type="button" value="게시글작성" onclick="document.location.href='bbInputPage?bbiCode=${bbiCode}&prdCode=${prdCode}'">
	            </c:if>
            </div>
          </section>
          <div class="paging">
            	<a id = "preButton" href="info_boardList?bbiCode=${bbiCode}&pageNo=${cutPageNo-1}&cutPageNo=${cutPageNo-5}">&#60;</a>
            	<c:choose>
            		<c:when test="${maxPage > cutPageNo+4}">
	            		<c:forEach var = "i" begin = "${cutPageNo}" end = "${cutPageNo+4}">
		            		<a href="info_boardList?bbiCode=${bbiCode}&pageNo=${i}&cutPageNo=${cutPageNo}" id="pageNo${i}"class="num">${i}</a>
		            	</c:forEach>            		
            		</c:when>
            		<c:otherwise>
                ...
                <a id = "nextButton" href="info_boardList?bbiCode=${bbiCode}&pageNo=${cutPageNo+5}&cutPageNo=${cutPageNo+5}">></a>
          </div>
        </div>
      </div>
      <aside id="aside2">
